...
public class FileReader {
  ...
  public func read(offset: UInt64, length: UInt32) async throws -> Data {
    ...
    repeat {
      ...
      // No forward progress (empty, non-EOF read — e.g. a malformed/short
      // response): stop instead of re-issuing the identical 0-byte read forever.
      if response.buffer.isEmpty { break }
  ...
  public func download(progressHandler: (_ progress: Double) -> Void = { _ in }) async throws -> Data {
    ...
    repeat {
      ...
      if response.buffer.isEmpty { break }   // no forward progress — avoid an infinite 0-byte loop
