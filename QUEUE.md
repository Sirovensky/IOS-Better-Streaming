...
2. **Radio tab (local-library stations)** — Apple-Music-style Radio but over the user's own library. New tab between **Home and Library** (order: Home, Radio, Library, Search). Auto-generate stations from metadata: by artist (works now), by genre, and "similar to <seed>" (shared genre/artist, reuse autoplayScore). Station = endless/large shuffled queue (`engine.playShuffled`, append more for endless). NOTE: genre is "Unknown" until embedded genre tags are extracted (format-specific id3/iTunes metadata) — ship artist/similar stations first, add genre after tag extraction.
